...
| server_type | 覆盖 | 路由要点 |
| --- | --- | --- |
| `stock_data` | **A 股** | 行情（快照 / K 线 / 分钟）+ 6 个 NL 专项（档案 / 财务 / 股东 / 事件 / 技术 / 风险） |
| `global_stock_data` | **港股 / 美股** | 与 `stock_data` 同结构，工具名加 `global_` 前缀 |
| `fund_data` | 基金 / ETF / LOF | 行情 + 6 个 NL 专项（档案 / 财务 / 持仓 / 业绩 / 持有人 / 管理公司） |
| `index_data` | 指数 / 板块 | 行情 + 3 个 NL 专项（档案 / 基本面 / 技术） |
| `bond_data` | 债券 | **无行情快照工具**，全走 NL（档案 / 主体 / 行情估值 / 主体财务） |
| `financial_docs` | 公告 + 新闻 | 2 个 RAG 工具（公告 / 新闻） |
| `economic_data` | 宏观 / 行业指标 | 1 个 EDB 工具 |
| `analytics_data` | 通用兜底 | 只在上述 7 个 server_type 都无法覆盖时使用 |

具体工具名见 `references/tool-manifest.json`；CLI 在 `call` 前本地校验 `server_type + tool_name` 组合，非法直接拦截，不会打到后端。
...
`cli.mjs` 用 **exit code** 区分成功/失败；不要从 stderr 解析任何东西（stderr 仅做内部日志和更新检查信号）。

- **exit code 0**：stdout 输出纯数据，无 envelope 包裹（`call` 透传 MCP `result` 对象；`open-portal` / `setup-key` 输出结构化 JSON）。
- **exit code 非 0**：stdout 输出 `{ ok: false, error: { code, agent_action } }`。先按 `error.code` 选分支策略（见第 7 节），再执行 `error.agent_action`。错误码字典见 `references/error-codes.json`。
...
> **⚠️ Shell 转义是 `INVALID_PARAMS_JSON` 的首要原因。** 首次调用前先确认当前 shell：`node -e "const s=process.env.SHELL||'',c=process.env.COMSPEC||'';console.log(/bash/i.test(s)?'bash':/powershell/i.test(process.env.PSModulePath?s:c)?'powershell':/cmd/i.test(c)?'cmd':'bash')"`，然后只按对应行写 JSON：
>
...
>
> 不要混用。NL 参数中禁止空格，用标点或直接连接（如 `'{\"question\":\"海光信息688041公司基本资料、所属行业\"}'`）。
...
不同自然语言工具的字段名不同，必须以 `## 3. 工具表` 中具体工具的参数表为准；禁止把 `query`、`question` 互相替换。
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `question` | ✅ | **自然语言问句**（不是指标 ID），如 `"中国GDP"` / `"美国CPI同比"` |
...
```bash
node scripts/cli.mjs call economic_data natural_language_get_edb_data '{"executionMode":"searchFetch","question":"中国CPI同比","beginDate":"20240101","endDate":"20261231"}'
```
...
**字段名硬约束**：`analytics_data.get_financial_data` 和 `economic_data.natural_language_get_edb_data` 只接受 `question` 作为自然语言入参；禁止使用 `query`。`query` 仅用于 `financial_docs.get_company_announcements` / `financial_docs.get_financial_news`。
...
对自然语言文本字段（`question` / `query`）的值，**禁止包含空格**。必须使用标点符号（顿号、逗号、括号等）分隔或直接连接。即使用户原句含空格，也必须在调用前去除所有空格；但字段名必须按具体工具表填写，不得混用。
...
| `aftime` 只接受 `"0"` / `"1"`（无"不复权"） | 其他值报错 |
| A 股查 `stock_data`，港股 / 美股查 `global_stock_data`，**别混** | A 股财务工具会拒港股 / 美股 |
| `server_type + tool_name` 必须存在于 `references/tool-manifest.json` | CLI 本地拦截，按 `error.agent_action` 修正即可 |
...
`cli.mjs` 失败时（exit code != 0）输出 envelope：`{ ok:false, error:{code, agent_action} }`。所有错误信息只解析 stdout，不从 stderr 取。
...
| `UNKNOWN_TOOL_NAME` / `UNKNOWN_SERVER_TYPE` | CLI 本地拦截（不会打到后端），按 `error.agent_action` 重选合法组合即可。 |
...
**`analytics_data` fallback 前提**：仅在 `PARAM_VALIDATION_ERROR` / `NO_RESULTS` 等"参数/数据覆盖"类错误且已尝试修正后才允许；先把复杂问题拆成简单结构化取数,不要机械照搬复杂原话。`analytics_data` 自身失败（非 Key / 权限 / 网络 / 5xx）可调整 `question` 措辞重试一次,不得改变用户原始意图;再失败则停止 fallback,转入终极兜底（见下）。
...
| 工具不存在 / 未知 server_type / 未知 tool_name / schema 对不上 | CLI 本地拦截，按 `error.agent_action` 修正 `server_type` / `tool_name` 后重试；仍不通过建议升级 skill |
...
CLI 在 stderr 输出更新检查信号（检测到新版 / 检测失败），每个会话**每种信号最多出现一次**，与 stdout 主调用结果完全独立。看到时简短转告用户即可（升级命令照搬 stderr 原文，不要自己改），不影响主调用判断。
