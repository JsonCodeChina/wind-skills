...
2. 配置 **WIND_API_KEY**：
   - 优先级：`WIND_API_KEY` 环境变量 > 本 skill 目录 `config`（dotenv：`WIND_API_KEY=...`） > `%USERPROFILE%\.wind-aifinmarket\config`（dotenv：`WIND_API_KEY=...`）。
...
- 绝不要输出真实 `WIND_API_KEY`、Bearer token、`config` 内容或 `%USERPROFILE%\.wind-aifinmarket\config` 内容。
