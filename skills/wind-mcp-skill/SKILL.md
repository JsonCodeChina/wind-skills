...
`get_financial_news`（获取来自第三方媒体的财经新闻报道，涵盖公司、行业及市场相关内容。返回由记者及分析师撰写发布的文章，包括市场评论、业绩报道、行业分析、财经快讯等。不包含公司官方发布的公告或监管披露文件）
...
| 字段 | 必填 | 类型 | 说明 |
|---|---|---|---|
| `question` | ✅ | string | 简洁自然语言取数问题，尽量保留用户原话 |
| `lang` | | enum | `CNS`=中文（默认）/ `ENS`=英文 |

使用要求：

- 优先将用户原始问句作为 `question` 透传调用一次；若失败、返回不匹配或问题过复杂，再改写 / 拆分为更简单的取数问题。
- `question` 入参不要太复杂，应尽量是单一取数动作，不要过分补写用户没有表达的条件。复杂分析、归因、预测、多条件筛选后再分析等任务，先拆成多个简单取数问题，再由 AI 综合。
