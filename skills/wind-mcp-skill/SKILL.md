...
> 3 个工具共用 `windcode` 字段：直接传用户原话里的标的名（中文名 / 简称 / 代码均可），后端自动解析（`贵州茅台` → `600519.SH`、`小米集团` → `01810.HK`、`易方达蓝筹精选` → `005827.OF`），AI 无需自己查代码。**用户给短名 / 别名（如 `茅台` 可匹配贵州茅台 / 茅台股份 / 茅台啤酒等多只）时主动预防性反问「你问的是哪只？」——后端不会报歧义，会直接选一个，可能选错**。代码格式参考：A 股 `600519.SH` / `8XXXXX.BJ`、港股 `00700.HK`、场外基金 `005827.OF`、ETF/LOF `588200.SH` / `159915.SZ`。
