...
`windcode` 的类型和多标的传入方式以各工具合约及 Wind 后端实际支持为准；不对所有工具施加全局单标的限制。
...
`stock_data.search_stocks`（股票筛选）从 A 股、港股或美股中筛选符合条件的股票，返回股票代码列表。

触发条件：用户未指定具体股票，而是描述选股条件，例如市场、市值、涨跌幅、行业、交易所、上市地、
上市板、连续上涨 / 下跌、技术形态或其它筛选条件。

不要用于：已指定单只股票的行情 / 财务查询；基金、指数、债券筛选；需要返回字段值而非股票代码列表的取数。

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `question` | 是 | 自然语言选股问句；用户给出的市场条件必须原样保留；调用前移除空白字符，不得添加用户未给出的筛选条件 |
...
`市盈率(TTM)`，`近一年收益率` -> `近一年净值增长率`。

市值口径必须明确：

- 用户要求“不含限售股”、“流通口径总市值”或“总市值不含限售”，传 `总市值1`。
- 用户要求“含限售股”、“含限售总市值”或“总市值含限售”，传 `总市值2`。
- 用户只说“总市值”且上下文无法确定口径时，调用前必须询问用户选择“不含限售股（流通口径）”或“含限售股”，不得自行默认。
- 最终回答必须写明实际使用的字段及口径，不得将 `总市值1` 和 `总市值2` 都简写为无口径的“总市值”。
...
- 股票：`换手率`、`量比`、`委比`、`涨停价`、`跌停价`、`52周最高`、
  `52周最低`、`总市值1`（不含限售股 / 流通口径）、`总市值2`（含限售股口径）、`流通市值`、`市盈率(TTM)`、`市净率`、`股息率`
