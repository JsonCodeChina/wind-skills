...
`windcode` 优先传用户给出的单个标的名称、简称或代码。Wind 后端会做标的 NER，可解析中文名、简称和标准代码；没有确定映射时，不得自行给名称或裸 ticker 补交易所后缀，也不得把自然语言标的猜成代码。只有用户明确给出标准代码和市场时，才使用带后缀的 Wind 标准代码，例如：
...
简称或别名可能映射多个标的时先问用户，不要让后端静默选错。行情类 NER 失败后，保持同一工具，只把 `windcode` 改成更明确的单个名称或用户确认的标准代码；禁止通过猜测 `.O`、`.N`、`.HK`、`.SH`、`.SZ` 等后缀来重试。

裸 6 位数字代码（如 `000001`）必须视为歧义标的，不得自动补后缀，也不得直接交给后端静默选择。因为它可能对应不同市场或品类，例如 `000001.SH`、`000001.SZ`、基金代码或其它标的。用户只给裸数字时，先要求补充标的全称、市场/品类，或明确 Wind 标准代码。
...
`.N`、`.HK`、`.SH`、`.SZ` 等交易所后缀；不确定时传用户原始单标的名称，让 Wind NER 识别。
