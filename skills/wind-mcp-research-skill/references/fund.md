...
> 本文件由 `scripts/registry.json` 生成（vserver_fund_research，21 个工具 / 64 个参数），不要手改。
...
| `fund_get_return_attribution` | 根据基金代码、基准及分析区间查询基金多因子模型分析结果，分析基金收益变化的主要因子来源。 | 模型因子只覆盖所选模型纳入的因子，不等同于行业持仓归因 | **windCode**, **benchmarkWindCode**, **startDate**, **endDate**, modelType, cycle, marketIndex, riskRate | `{"windCode":"510300.OF","benchmarkWindCode":"000300.SH","startDate":"2025-09-01","endDate":"2026-09-01"}` |
...
| `fund_get_nav` | 获取单只或多只基金截至指定查询日期可取得的时点单位净值。 | 仅返回时点值，不提供历史或区间净值序列、分红拆分折算、区间收益、排名评级、风险指标、规模份额、场内行情或申赎状态 | **windCodes**, asOfDate | `{"windCodes":["510300.OF","000001.OF"],"asOfDate":"2026-09-03"}` |
...
| `fund_get_size` | 根据基金代码和指定日期或报告期查询基金规模信息。 | 最新时点规模与报告期规模不能混用 | **windCodes**, asOfDate | `{"windCodes":["510300.OF","000001.OF"],"asOfDate":"2026-09-03"}` |
