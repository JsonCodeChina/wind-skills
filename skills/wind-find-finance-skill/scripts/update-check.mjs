...
// ───── 通知 (find-finance 无 cli, 探测后同进程内联通知) ─────

// 读 cache → 有 pending 则 stderr 通知一次 + 标记已通知 (lastNotifiedSha=latestSha → 同版本不再重复)
// 失败完全静默 (探活失败由 main 吞掉, 这里只读 cache 结果)
...
// find-finance 无 cli: SKILL.md 直接 `node update-check.mjs` 同步探测 → 探测完同进程内联通知
async function mainAndNotify() {
  await main();          // 探测 + 写 cache (静默)
  maybeNotifyUpdate();   // 读 cache + 通知 pending + 标记 + 长尾兜底
}

const IS_MAIN = process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href;
if (IS_MAIN) mainAndNotify().catch(() => {});
