...
> 何时读：选择 server_type=stock_data 后读对应段落 | 权威于：工具字段、参数、场景、示例
...
| tool_name | 入参 |
| --- | --- |
| `search_stocks` | `question`（+`lang` / `version`） |
| `get_stock_price_indicators` | `windcode` + `indexes` |
| `get_stock_kline` | `windcode` + `begin_date` + `end_date`（+`period` / `count` / `aftime`…） |
| `get_stock_quote` | `windcode`（+`begin` / `end`） |
| `get_stock_basicinfo` / `get_stock_fundamentals` / `get_stock_equity_holders` / `get_stock_events` / `get_stock_technicals` / `get_risk_metrics` | `question`（+`lang`） |
...
| --- | --- | --- |
| 行情类 | `{windcode, ...}` | 股票行情工具 |
| 股票筛选 | `{question, lang?, version?}` | `search_stocks` |
